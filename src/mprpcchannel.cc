...
void MprpcChannel::CallMethod(const google::protobuf::MethodDescriptor* method,google::protobuf::RpcController* controller, const google::protobuf::Message* request,google::protobuf::Message* response, google::protobuf::Closure* done)
{
    ...
    //连接rpc服务节点
    if(-1 == connect(cilentfd,(struct sockaddr*)&server_addr,sizeof(server_addr))){
        close(cilentfd);
        char errtext[512] = {0};
        sprintf(errtext,"create connection error! errno: %d",errno);
        controller->SetFailed(errtext);
        return;
    }

    //发送rpc请求
    if(-1 == send(cilentfd,send_rpc_str.c_str(),send_rpc_str.size(),0)){
        close(cilentfd);
        char errtext[512] = {0};
    ...
    int recv_size = 0;
    if(-1 == (recv_size =recv(cilentfd,recv_buf,1024,0))){
        close(cilentfd);
        char errtext[512] = {0};
    ...
    //反序列化rpc调用的响应数据
    // std::string response_str(recv_buf,0,recv_size);//recv_buf中遇到\0后面的数据就存不下来，导致反序列化失败
